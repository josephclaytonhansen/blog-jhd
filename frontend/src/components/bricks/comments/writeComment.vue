...
<template>
    <div v-if="writing && props.blogPost && props.user" class="w-full flex flex-col items-start">
        ...
        <button @click="uploadComment" class="cursor-pointer bg-accent-500 px-5 py-2 rounded-lg text-text-0 hover:bg-accent-600 hover:scale-105 transition-all duration-300">Submit</button>
    </div>
    <div v-else-if="!writing && props.user !== '' && props.user !== null && props.user !== undefined && props.user && props.blogPost" class="w-full">
        <button @click="writing = true" class="cursor-pointer bg-accent-500 px-5 py-2 rounded-lg w-full text-text-0 hover:bg-accent-600 hover:scale-105 transition-all duration-300">Write a comment</button>
    </div>
    <div v-else class="w-full items-start">
        <p class=" w-full mb-2">Log in or register to write a comment</p>
        <router-link to="/login" class="cursor-pointer bg-accent-500 px-5 py-2 rounded-lg text-text-0 hover:bg-accent-600 hover:scale-105 transition-all duration-300">Log in</router-link>
    </div>
